...
def solar_solver(E, beta, tau, del_m2, theta, n_slabs=500000, r_i=0.0, r_f=1.0, renorm_every_step=True):
    E = E * 1e6
    dx = (r_f - r_i) * R_earth / n_slabs
    r_vals = np.linspace(r_i + dx/(2*R_earth), r_f - dx/(2*R_earth), n_slabs)
    psi = np.array([1.0, 0.0, 0.0], dtype=float)
    ...
    def f(r, psi_vec):
        M = build_M(r)
        return -2.0 * (M @ psi_vec)

    # RK4 loop
    for i in range(n_slabs):
        r_mid = r_vals[i] * R_earth
        k1 = f(r_mid, psi)
        k2 = f(r_mid + 0.5*dx, psi + 0.5*dx*k1)
        k3 = f(r_mid + 0.5*dx, psi + 0.5*dx*k2)
        k4 = f(r_mid + dx, psi + dx*k3)

        psi = psi + (dx/6.0) * (k1 + 2.0*k2 + 2.0*k3 + k4)

        if renorm_every_step:
            curr_norm = np.linalg.norm(psi)
            psi *= (norm0 / curr_norm)
...
# ------------------------- Chi2 Grid Search ------------------------- #
dm2_grid = np.logspace(-4.52, -3.69, 20)
tan2theta_grid = np.logspace(-1, 0, 20)

# ------------------------- Beta Variation Setup ------------------------- #
beta_list = [0,0.02,0.04,0.06]  # Add more β values as desired
